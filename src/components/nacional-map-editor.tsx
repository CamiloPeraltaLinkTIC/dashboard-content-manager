...
const PLATFORMS = ["TikTok", "X", "Instagram", "Facebook"] as const;
...
// Suma de plataformas y plataforma dominante de una fila editable
const platSum = (plats: Record<string, number>) =>
  PLATFORMS.reduce((acc, p) => acc + (Number(plats?.[p]) || 0), 0);
const dominantPlat = (plats: Record<string, number>) =>
  PLATFORMS.reduce((a, b) => ((Number(plats?.[b]) || 0) > (Number(plats?.[a]) || 0) ? b : a), "TikTok" as Platform);
...
export function NacionalMapEditor({ data, onSaved }: { data: any[]; onSaved: () => void }) {
  ...
  useEffect(() => {
    setRows(
      data.map((d) => ({
        ...d,
        sentimientoPct: { positivo: 0, neutral: 0, negativo: 0, ...(d.sentimientoPct || {}) },
        plataformas: { TikTok: 0, X: 0, Instagram: 0, Facebook: 0, ...(d.plataformas || {}) },
      }))
    );
  }, [data]);
  ...
  const updatePlat = (id: string, plat: Platform, value: any) =>
    setRows((prev) =>
      prev.map((r) => {
        if (r.id !== id) return r;
        const plataformas = { ...r.plataformas, [plat]: parseInt(String(value)) || 0 };
        return { ...r, plataformas, volumen: platSum(plataformas) };
      })
    );

  // Descarga una plantilla .xlsx con la estructura correcta y los departamentos
  ...
  const downloadTemplate = () => {
    const aoa = [
      ...
      ...rows.map((r) => [
        ...
        r.nombre,
        r.capital || "",
        r.plataformas?.TikTok ?? 0,
        r.plataformas?.X ?? 0,
        r.plataformas?.Instagram ?? 0,
        r.plataformas?.Facebook ?? 0,
    ...
    XLSX.writeFile(wb, "plantilla_conversacion_nacional.xlsx");
  };

  // Importa un Excel y calza cada fila contra los departamentos por Codigo (DANE)
    ...
    reader.onload = (evt) => {
      try {
        ...
        excelRows.forEach((row) => {
          ...
          const plataformas = { ...target.plataformas };
          let touchedPlat = false;
          PLATFORMS.forEach((p) => {
            if (has(p)) { plataformas[p] = parseNum(row[p]); touchedPlat = true; }
          });
          if (touchedPlat) { target.plataformas = plataformas; target.volumen = platSum(plataformas); }
  ...
  const save = async () => {
    ...
    try {
      for (const r of rows) {
        const plataformas = {
          TikTok: parseInt(String(r.plataformas?.TikTok || 0)) || 0,
          X: parseInt(String(r.plataformas?.X || 0)) || 0,
          Instagram: parseInt(String(r.plataformas?.Instagram || 0)) || 0,
          Facebook: parseInt(String(r.plataformas?.Facebook || 0)) || 0,
        };
        const volumen = platSum(plataformas);
        const payload = {
          id: r.id,
          nombre: r.nombre,
          capital: r.capital || "",
          ...
          volumen,
          plataforma_dominante: dominantPlat(plataformas),
          plataformas,
          resumen: r.resumen || "",
        ...
        const { error } = await supabase
          .from("content_manager_nacional_departamentos")
        ...
        {rows.map((r) => (
          <details key={r.id} className="group bg-[#0b101d] border border-white/10 rounded-xl overflow-hidden">
            ...
            <div className="p-4 pt-2 border-t border-white/5 grid grid-cols-2 gap-3">
              ...
              <Field label="Volumen total (auto)">
                <Input type="number" disabled className={`${inputCls} text-yellow-500 font-bold opacity-70`} value={r.volumen ?? 0} />
              </Field>
              <Field label="% Cambio">
